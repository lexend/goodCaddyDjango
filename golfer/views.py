...
def golfer(request):#this section for my appointment
	...
	if "Golfer" == group_name:
		...
		return render(request, 'golfer.html', appointments )
	else:
		return redirect('http://127.0.0.1:8000/')

def golfer_appointment_list(request):
	group_name=Group.objects.all().filter(user = request.user)# get logget user grouped name
	group_name=str(group_name[0]) # convert to string
	if "Golfer" == group_name:
		user_name=request.user.get_username()#Getting Username
		...
		return render(request, 'golfer_create_appointment.html', appointments )
	else:
		return redirect('http://127.0.0.1:8000/')
...
def golfer_appointment_update(request,id):
	group_name=Group.objects.all().filter(user = request.user)# get logget user grouped name
	group_name=str(group_name[0]) # convert to string
	if "Golfer" == group_name:
		user_name=request.user.get_username()#Getting Username
		...
		if form.is_valid():
			    ...
			    saving.save()
			    messages.success(request, 'Post Created Sucessfully')
			    return redirect('http://127.0.0.1:8000/golfer/create_appointment/')
		...
		return render(request, 'golfer_appointment_update.html', appointments )
	else:
		return redirect('http://127.0.0.1:8000/')
