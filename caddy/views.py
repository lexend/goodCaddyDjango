...
from golfer.models import Appointment
from golfer.forms import AppointmentForm
...
def quick_appointmnet(request):
	...
	if "Caddy" == group_name:
		...
		return render(request, 'caddy_quick_appointment.html', appointments )
	else:
		return redirect('http://127.0.0.1:8000/')


def caddy(request):#this section for my appointment
	group_name=Group.objects.all().filter(user = request.user)# get logget user grouped name
	group_name=str(group_name[0]) # convert to string
	if "Caddy" == group_name:
		...
		return render(request, 'caddy.html', appointments )
	else:
		return redirect('http://127.0.0.1:8000/')
